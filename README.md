...
- `scripts/check_project_memory.py`：对已初始化的项目做记忆新鲜度和 `feature_list.json` 结构的提醒性检查（不会让检查失败，只打印警告）。
...
- `CLAUDE.md`：如果项目主要用 Claude Code，这个文件通过 `@AGENTS.md` 让 Claude Code 自动加载上面的项目指令（Claude Code 默认不会自动读取 `AGENTS.md`）。
...
4. 任务推进到关键节点时更新进度和决策，进度记录只增补不回改，条目多了就归档到 `task_state/archive/`。
5. 结束时写清楚验证结果、未完成事项和下一步。
6. 定期（比如每隔几周）在项目里跑一次 `python scripts/check_project_memory.py --target .`，看看有没有过期或格式跑偏的记忆条目。
